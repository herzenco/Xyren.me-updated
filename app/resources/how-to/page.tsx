...
export default async function HowToPage() {
  ...
  // Fetch published guides from Supabase
  const { data: guides, error } = await (supabase.from('how_to_guides') as any)
    .select('*')
                ...
                <Card className="h-full hover:border-primary/50 hover:shadow-md transition-all">
                  <CardContent className="p-6 space-y-3">
                    <div className="flex items-center gap-2">
                      <span className={`text-xs font-medium px-2.5 py-1 rounded-full capitalize ${difficultyColors[guide.difficulty as keyof typeof difficultyColors]}`}>
                        {guide.difficulty}
                      </span>
